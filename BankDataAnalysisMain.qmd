...
Data %>% 
  group_by(y) %>% 
  ggplot(aes(x = y, y = balance, fill = y)) + 
  geom_boxplot() +
  scale_y_log10(labels = comma) +
  labs(title = "Związek balansu konta z chęcia założenia lokaty", y = "Balans konta") +
  theme_bw() +
...
Data %>%
  ...
  geom_bar(position = "fill")+
  theme_bw()+
  labs(title = "Chęć założenia lokaty w zależności od stanu cywilnego", x = "", y = "")+
  theme(plot.title = element_text(hjust=0.5))
```
...
pie(d$n, labels = c("FALSE", "TRUE"), radius = 1, col = c("orange", "blue"), border = "white", main = "Podział wartości y")
```
...
-   Logistic Regression Do tunowania hiperparametrów wykorzystywaliśmy walidację krzyżową.
...
Na pierwszy rzut oka widać, że Boosting jest najbardziej efektywnym modelem. Zaraz po nim najlepiej z zadaniem klasyfikacji radzi sobie Random Forest.
...
Na wykresie krzywych roc auc najgorzej wyszedł model KNN. Najlepiej wspomniany wcześniej XGBoost.

```{r}
load("wykresy/workflows_acc.rda")
workflows_acc
```

```{r}

load("wykresy/workflows_roc_auc.rda")
workflows_roc_auc
```
...
Przy tworzeniu tego projektu nauczyliśmy się rozwiązywać niespotkane do tej pory problemy, takie jak niezbalansowanie danych czy praca pod presją czasu ;). Nauczyliśmy się pracy z biblioteką tidymodels. Poznaliśmy nowe modele uczenia maszynowego oraz poprawiliśmy umiejętności pracy w grupie.
