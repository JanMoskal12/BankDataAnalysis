...
```{r}
rf_fit <- readRDS("wf_rf_best_fit.rds")
...
svm_fit <- readRDS("wf_best_svm_fit.rds")
...
knn_fit <- readRDS("wf_knn_best_fit.rds")
...
tree_fit <- readRDS("wf_tree_best_fit.rds")
...
log_fit <- readRDS("wfbest_regresja_train.rds")
...
boost_fit <- readRDS("wfbest_boosting_fit.rds")
...
curve <- ggplot()+
  geom_path(data=rf_roc,aes(x = 1 - specificity, y = sensitivity, col="#e41a1c"))+
  ...
  scale_color_identity(name = "Model",
                       breaks = c("#e41a1c", "#377eb8","#4daf4a","#984ea3","#ff7f00","#ffff33"),
